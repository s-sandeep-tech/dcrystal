...
def get_external_db_connection():
    ...
    host = "crystal-replica1.kalyanjewellers.company"
    ...
    try:
        ...
        for row in rows:
            new_records.append({
                ...
                'branch_type': row.get('branch_type') or '',
                'customer_order_type': row.get('customer_order_type') or '',
                'is_discount_party': row.get('is_discount_party') or '',
...
def sync_pending_order_details_task(task_type_override=None, progress_range=(0, 100), is_subtask=False) -> Dict[str, Any]:
    conn = None
    TASK_TYPE = task_type_override or 'pending_order_details'
    
    def emit(status, message, progress):
        emit_combined_sync_update(status, message, progress, TASK_TYPE, progress_range, is_subtask)

    try:
        emit('processing', 'Starting Pending Order Details Sync...', 5)
        conn = get_external_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        
        emit('processing', 'Fetching data from Azure PostgreSQL...', 20)
        query = "SELECT * FROM ext_view.vw_pending_order_details"
        ...
        logger.info(f"vw_pending_order_details query took {duration:.2f} seconds.")
        ...
        for row in rows:
            new_records.append({
                ...
                'is_discount_party': row.get('is_discount_party'),
                'accept_pending_pcs': row.get('accept_pending_pcs') or 0,
                'accept_pending_wt': row.get('accept_pending_wt') or 0.0,
...
def _provision_sync_producer(conn_params, data_queue, stop_event, batch_size, total_to_sync, shared_state):
    ...
    query_template = """
        SELECT "division","group","location","branch_id","provision_mode","provision_mode_filter",
               ...
               "state","division_id","group_id","purity_id","classification_id",
               "sub_classification_id","section_id","type_id","make_id","collection_id",
               "master_collection_id","sub_section_id","wide_range_id","gender_id","size_id",
               "screw_type_id",
               "last_updated_at"
    ...
    try:
        ...
        try:
            ...
            copy_query = """
                ...
                gender, wide_range, size, screw_type, division_id, group_id, purity_id, classification_id,
                sub_classification_id, section_id, type_id, make_id, collection_id, master_collection_id,
                sub_section_id, wide_range_id, gender_id, size_id, screw_type_id,
                prov_type, purity, range_weight, prov_pieces,
...
def sync_size_level_nip_barcode_task() -> Dict[str, Any]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            emit_sync_update(
                'processing',
                f'Starting Size Level NIP Barcode Sync (Attempt {attempt}/{max_attempts})...',
                5,
                data_type,
            )
            conn = get_external_db_connection()
            ...
            logger.error(
                f'Size Level NIP Barcode Sync failed '
            ...
            emit_sync_update(
                'processing',
                f'Sync failed; retrying in {retry_delay_seconds}s '
            ...
            if conn:
                try:
                    ...
                    logger.warning(
                        f'Failed to close Size Level NIP Barcode connection: {connection_close_error}'
                    )
...
def sync_party_delay_management_data_task():
    ...
    while retry_count < max_retries:
        ...
        try:
            ...
            db.session.rollback()
            retry_count += 1
            error_msg = f"PartyDelayManagement sync failed (Attempt {retry_count}/{max_retries}): {str(e)}"
            logger.error(error_msg)
        ...
        try:
            ...
            for row in source_rows:
                records.append({
                    ...
                    'correction_pcs': to_int(row['correction_pcs']) if row.get('correction_pcs') is not None else None,
                    'make_owner': str(row['make_owner']).strip() if row.get('make_owner') is not None else None,
                    'collection_owner': str(row['collection_owner']).strip() if row.get('collection_owner') is not None else None,
                    'collection_owner_emp_code': str(row['collection_owner_emp_code']).strip() if row.get('collection_owner_emp_code') is not None else None,
                    'make_owner_emp_code': str(row['make_owner_emp_code']).strip() if row.get('make_owner_emp_code') is not None else None,
...
def sync_sales_stock_composition_analysis_task(task_type_override=None, progress_range=(0, 100), is_subtask=False) -> Dict[str, Any]:
    TASK_TYPE = task_type_override or 'sales_stock_composition_analysis'
    ...
    for attempt in range(1, max_attempts + 1):
        conn = None
        try:
            emit('processing', f'Starting Sales & Stock Composition Analysis Sync (Attempt {attempt}/{max_attempts})...', 5)

            conn = get_external_db_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)

            emit('processing', 'Fetching data from vw_item_wise_tsk_details_data...', 20)
            started_at = time.time()
            cursor.execute("SET statement_timeout = 0")
            
            try:
                query = "SELECT * FROM ext_view.vw_item_wise_tsk_details_data"
                cursor.execute(query)
            except Exception:
                query = "SELECT * FROM vw_item_wise_tsk_details_data"
                cursor.execute(query)

            source_rows = cursor.fetchall()
            duration = time.time() - started_at

            logger.info(f"vw_item_wise_tsk_details_data query took {duration:.2f} seconds.")
            emit('processing', f'Fetched {len(source_rows):,} records in {duration:.1f}s. Updating local snapshot...', 50)

            SalesStockCompositionAnalysisSnapshot.__table__.create(db.engine, checkfirst=True)
            db.session.query(SalesStockCompositionAnalysisSnapshot).delete()
            ...
            db.session.rollback()
            logger.exception('Sales Stock Composition Analysis sync failed (attempt %s/%s)', attempt, max_attempts)
            if attempt == max_attempts:
                message = f'Sync failed after {max_attempts} attempts: {exc}'
                emit('error', message, 0)
                return {'status': 'error', 'message': message}
