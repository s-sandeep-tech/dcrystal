...
@dashboard_bp.route('/settings/sync-pending-order-details', methods=['POST'])
def sync_pending_order_details():
    if not session.get('user_id') or ('ADMIN' not in session.get('roles', []) and 'DATA_SYNC_USER' not in session.get('roles', [])):
        return {"status": "error", "message": "Unauthorized: Admin or Data Sync role required"}, 401
        
    from app.utils.sync_manager import sync_pending_order_details_data
    result = sync_pending_order_details_data(session.get('user_id'))
    return result, 200 if result.get('status') == 'success' else 500
...
@dashboard_bp.route('/settings/sync-party-make-capacity-details', methods=['POST'])
def sync_party_make_capacity_details():
    if not session.get('user_id') or ('ADMIN' not in session.get('roles', []) and 'DATA_SYNC_USER' not in session.get('roles', [])):
        return {"status": "error", "message": "Unauthorized: Admin or Data Sync role required"}, 401
        
    from app.utils.sync_manager import sync_party_make_capacity_details_data
    result = sync_party_make_capacity_details_data(session.get('user_id'))
    return result, 200 if result.get('status') == 'success' else 500
...
@dashboard_bp.route('/settings/sync-qc-completed-invoice-request-pending', methods=['POST'])
def settings_sync_qc_completed_invoice_request_pending():
    if not session.get('user_id') or ('ADMIN' not in session.get('roles', []) and 'DATA_SYNC_USER' not in session.get('roles', [])):
        return {"status": "error", "message": "Unauthorized"}, 401
    from app.utils.sync_manager import sync_qc_completed_invoice_request_pending_data
    result = sync_qc_completed_invoice_request_pending_data(session.get('user_id'))
    return result, 200 if result.get('status') == 'success' else 500
...
def settings_sync_qc_receipt_completed_pending():
    if not session.get('user_id') or ('ADMIN' not in session.get('roles', []) and 'DATA_SYNC_USER' not in session.get('roles', [])):
        return {"status": "error", "message": "Unauthorized"}, 401
    from app.utils.sync_manager import sync_qc_receipt_completed_pending_data
    result = sync_qc_receipt_completed_pending_data(session.get('user_id'))
    return result, 200 if result.get('status') == 'success' else 500

@dashboard_bp.route('/settings/sync-party-delay-management', methods=['POST'])
def settings_sync_party_delay_management():
    if not session.get('user_id') or ('ADMIN' not in session.get('roles', []) and 'DATA_SYNC_USER' not in session.get('roles', [])):
        return {"status": "error", "message": "Unauthorized"}, 401
    from app.utils.sync_manager import sync_party_delay_management_data
    result = sync_party_delay_management_data(session.get('user_id'))
    return result, 200 if result.get('status') == 'success' else 500
