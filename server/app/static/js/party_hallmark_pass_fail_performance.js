let currentZoom = parseFloat(localStorage.getItem('party-hm-zoom')) || 1.0;
let currentPage = 1;
let totalPages = 1;
...
function adjustZoom(delta, reset = false) {
    ...
    localStorage.setItem('party-hm-zoom', currentZoom);
...
async function loadViewData() {
    const activeView = document.getElementById('view-party-hm');
    if (!activeView) return;

    const searchParams = buildQueryParams();

    try {
        const response = await fetch(`/partial/party-hallmark-pass-fail-performance?${searchParams}`, {
            headers: {
...
function updateHeaderStats(stats) {
    if (!stats) return;
    const issuePcs = document.getElementById('stat-hm-issue-pcs');
    const issueWt = document.getElementById('stat-hm-issue-wt');
    const passedPcs = document.getElementById('stat-hm-passed-pcs');
    const passedWt = document.getElementById('stat-hm-passed-wt');
    const failedPcs = document.getElementById('stat-hm-failed-pcs');
    const failedWt = document.getElementById('stat-hm-failed-wt');

    if (issuePcs) issuePcs.textContent = (stats.hm_issue_pcs || '0') + ' Pcs';
    if (issueWt) issueWt.textContent = (stats.hm_issue_wt || '0.000') + ' Wt';
    if (passedPcs) passedPcs.textContent = (stats.hm_passed_pcs || '0') + ' Pcs';
    if (passedWt) passedWt.textContent = (stats.hm_passed_wt || '0.000') + ' Wt';
    if (failedPcs) failedPcs.textContent = (stats.hm_failed_pcs || '0') + ' Pcs';
    if (failedWt) failedWt.textContent = (stats.hm_failed_wt || '0.000') + ' Wt';
}
...
function buildQueryParams() {
    ...
    const filterIds = [
        ['filter-party', 'party'],
        ['filter-hallmarking-center', 'hallmarking_center'],
    ...
    if (failedOnly?.checked) params.set('failed_only', '1');
...
function resetAllFilters() {
    const filterIds = ['filter-party', 'filter-hallmarking-center', 'filter-order-type', 'filter-provision-type'];
...
async function togglePartyHallmarkRow(btn) {
    ...
    if (tr.classList.contains('expanded')) {
        ...
        while (nextRow && (
            level === 'party'
                ? nextRow.classList.contains('child-row')
                : nextRow.dataset.level === 'metric'
        )) {
    ...
    if (level === 'party') {
        params.set('target_level', 'hallmarking_center');
    } else {
        params.set('target_level', 'metric');
        params.set('parent_center', center);
    }

    try {
        const response = await fetch(`/partial/party-hallmark-pass-fail-performance?${params.toString()}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
        });
