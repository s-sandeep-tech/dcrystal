let currentZoom = parseFloat(localStorage.getItem('design-allocation-zoom')) || 1.0;
let currentPage = 1;
let totalPages = 1;
let currentSortBy = 'make';
...
function adjustZoom(delta, reset = false) {
    ...
    localStorage.setItem('design-allocation-zoom', currentZoom);
...
async function loadViewData() {
    const activeView = document.getElementById('view-design-allocation');
    if (!activeView) return;

    const searchParams = buildQueryParams();

    try {
        const response = await fetch(`/partial/design-allocation-performance?${searchParams}`, {
            headers: {
...
function resetAllFilters() {
    const filterIds = ['filter-make-owner', 'filter-make', 'filter-section', 'filter-wide-range', 'filter-order-type', 'filter-provision-type'];
...
async function toggleDesignAllocationRow(btn, level, make, section) {
    ...
    if (level === 'make') {
        params.set('target_level', 'section');
        params.set('parent_make', make || '');
    } else if (level === 'section') {
        params.set('target_level', 'wide_range');
        params.set('parent_make', make || '');
        params.set('parent_section', section || '');
    }

    try {
        const response = await fetch(`/partial/design-allocation-performance?${params.toString()}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
        });
