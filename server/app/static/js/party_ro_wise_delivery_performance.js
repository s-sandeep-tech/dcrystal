let currentZoom = parseFloat(localStorage.getItem('party-ro-zoom')) || 1.0;
let currentPage = 1;
let totalPages = 1;
...
function adjustZoom(delta, reset = false) {
    ...
    localStorage.setItem('party-ro-zoom', currentZoom);
...
async function loadViewData() {
    const activeView = document.getElementById('view-party-ro');
    if (!activeView) return;

    const searchParams = buildQueryParams();

    try {
        const response = await fetch(`/partial/party-ro-wise-delivery-performance?${searchParams}`, {
            headers: {
...
function updateHeaderStats(stats) {
    if (!stats) return;
    const totalRoCountEl = document.getElementById('stat-total-ro-count');
    const totalDeliveredWtEl = document.getElementById('stat-total-delivered-wt');

    if (totalRoCountEl) totalRoCountEl.textContent = stats.total_ro_count || '0';
    if (totalDeliveredWtEl) totalDeliveredWtEl.textContent = stats.total_delivered_wt || '0.000';
}
...
function resetAllFilters() {
    const filterIds = ['filter-party', 'filter-make', 'filter-make-owner', 'filter-order-type', 'filter-provision-type'];
...
async function togglePartyRoRow(btn, level, party) {
    ...
    try {
        const response = await fetch(`/partial/party-ro-wise-delivery-performance?${params.toString()}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
        });
